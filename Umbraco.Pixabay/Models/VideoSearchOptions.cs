...
namespace Umbraco.Pixabay.Models
{
    public class VideoSearchOptions
    {
        ...
        public VideoSearchOptions(SearchOptions options)
        {
            SearchTerm = options.SearchTerm;
            ApiKey = options.ApiKey;
            if (!string.IsNullOrEmpty(options.Language)) Language = (EnumLanguage)options.Language.ToEnum<EnumLanguage>(true);
            if (!string.IsNullOrEmpty(options.VideoType)) VideoType = (EnumVideoType)options.VideoType.ToEnum<EnumVideoType>(true);
        ...
        /// Default: "all"
        /// </summary>
        public EnumVideoType VideoType { get; set; } = EnumVideoType.All;
